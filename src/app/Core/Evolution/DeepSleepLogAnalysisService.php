...
final class DeepSleepLogAnalysisService
{
    ...
    public function analyze(Config $config): array
    {
        ...
        $outPath = BASE_PATH . '/storage/evolution/deep_sleep_last.json';
        $dir = dirname($outPath);
        if (!is_dir($dir)) {
            @mkdir($dir, 0755, true);
        }
    ...
    {
        $file = BASE_PATH . '/public/storage/logs/errors/' . $ymd . '.log';
        if (!is_file($file)) {
            return 0;
        }
    ...
    {
        $path = BASE_PATH . '/storage/evolution/evolution.log';
        if (!is_file($path)) {
            return [];
        }
