...
final class EvolutionMasterOpinionService
{
    ...
    public function evaluatePhpAndTests(
        ...
        $openaiKey = trim((string) $config->get('ai.openai.api_key', $config->get('assistant.api_key', '')));
        if ($openaiKey === '') {
            return [
                'ok' => false,
                'master_score' => 0.0,
                ...
                'error' => 'Missing ai.openai.api_key',
            ];
        }
...
SYS;
    ...
    public function critiqueDesignJson(string $designJson, Config $config): array
    {
        ...
        $openaiKey = trim((string) $config->get('ai.openai.api_key', $config->get('assistant.api_key', '')));
        if ($openaiKey === '') {
            return ['ok' => false, 'raw' => '', 'error' => 'Missing ai.openai.api_key'];
        }
...
SYS;
    ...
    private static function appendHallOfWisdom(array $out): void
    {
        ...
        $path = BASE_PATH . '/storage/evolution/hall_of_wisdom.jsonl';
        $tb = $out['toolbox'] ?? null;
        $line = json_encode([
            'ts' => gmdate('c'),
    ...
    public static function recordIntegrityMilestoneIfNeeded(Container $container): void
    {
        ...
        $flag = BASE_PATH . '/storage/evolution/.master_second_opinion_milestone_done';
        if (is_file($flag)) {
            return;
        }
