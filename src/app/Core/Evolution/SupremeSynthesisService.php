...
final class SupremeSynthesisService
{
    ...
    public function run(string $userTask, string $mode = 'core'): array
    {
        ...
        $anthropicKey = trim((string) ($config->get('evolution.anthropic.api_key', '')));
        if ($anthropicKey === '') {
            return ['ok' => false, 'error' => 'Missing evolution.anthropic.api_key / ANTHROPIC_API_KEY'];
        }
        $openaiKey = trim((string) $config->get('ai.openai.api_key', $config->get('assistant.api_key', '')));
        if ($openaiKey === '') {
            return ['ok' => false, 'error' => 'Missing ai.openai.api_key'];
        }
