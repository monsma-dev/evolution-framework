...
final class EvolutionArchivistService
{
    ...
    private function wasInCroRecently(string $absoluteTwig, Config $config): bool
    {
        ...
        $croPath = BASE_PATH . '/storage/evolution/cro_events.jsonl';
        if (!is_file($croPath)) {
            return false;
        }
    ...
    private function archiveDestination(string $absoluteSource): ?string
    {
        ...
        $day = gmdate('Y-m-d');
        $destDir = BASE_PATH . '/storage/evolution/archive/' . $day;
    ...
    {
        $path = BASE_PATH . '/storage/evolution/archive/manifest.jsonl';
        $dir = dirname($path);
        if (!is_dir($dir) && !@mkdir($dir, 0755, true) && !is_dir($dir)) {
            return;
        }
