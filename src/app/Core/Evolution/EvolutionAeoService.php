...
final class EvolutionAeoService
{
    ...
    public static function sync(Container $container): array
    {
        ...
        $kgPath = BASE_PATH . '/storage/evolution/knowledge_graph.json';
        $kg = [];
        if (is_file($kgPath)) {
        ...
        if (!is_dir($publicDir) && !@mkdir($publicDir, 0755, true) && !is_dir($publicDir)) {
            return ['ok' => false, 'error' => 'cannot create public/'];
        }
    ...
    public static function pulseCheck(Config $config): array
    {
        ...
        $llms = is_file(BASE_PATH . '/public/llms.txt');
        $robots = is_file(BASE_PATH . '/public/robots.txt');
        $kgPath = BASE_PATH . '/storage/evolution/knowledge_graph.json';
        $nodes = 0;
        if (is_file($kgPath)) {
