...
final class FigmaTailwindParser
{
    ...
    {
        [$fileKey, $nodeId] = $this->extractIds($figmaUrl);
        if ($fileKey === '') {
            return ['ok' => false, 'error' => 'Could not extract Figma file key from URL'];
        }

        return $this->parseNode($fileKey, $nodeId, $pat);
    }
    ...
    public function parseNode(string $fileKey, string $nodeId, string $pat): array
    {
        ...
        $raw = $this->apiGet($endpoint, $pat);
        if ($raw === null) {
            return ['ok' => false, 'error' => 'Figma API request failed (check FIGMA_ACCESS_TOKEN)'];
        }
    ...
    private function apiGet(string $url, string $pat): ?string
    {
        $ctx = stream_context_create([
            'http' => [
                'method'  => 'GET',
                'header'  => "X-Figma-Token: {$pat}\r\nUser-Agent: EvolutionDesignAgent/1.0",
    ...
    private function extractIds(string $url): array
    {
        ...
        if (preg_match('#figma\.com/(?:design|file)/([A-Za-z0-9]+)#', $url, $m)) {
            $fileKey = $m[1];
            $nodeId = '';
            if (preg_match('/node-id=([0-9]+)[:\-]([0-9]+)/', $url, $nm)) {
                $nodeId = $nm[1] . ':' . $nm[2];
            }
