...
PROMPT;
    ...
    {
        $pat = trim((string)(getenv('FIGMA_ACCESS_TOKEN') ?: $this->config->get('evolution.figma.access_token', '')));
        if ($pat === '') {
            return ['ok' => false, 'error' => 'FIGMA_ACCESS_TOKEN not configured'];
        }
        ...
        try {
            $client = new DeepSeekClient($apiKey);
            $html = $client->complete(self::SYSTEM_PROMPT, [
                ['role' => 'user', 'content' => $prompt],
            ], DeepSeekClient::MODEL_CHAT, 4096);
        } catch (\Throwable $e) {
            return ['ok' => false, 'error' => $e->getMessage()];
        }
