...
final class EvolutionFigmaCliHelper
{
    ...
    public static function status(Container $container): array
    {
        ...
        if ($token === '') {
            $out['ok'] = false;
            $out['error'] = 'Figma PAT ontbreekt: FIGMA_ACCESS_TOKEN, evolution.figma.api_token, of evolution.figma_bridge.access_token.';
    ...
    private static function figmaHttpJson(string $method, string $url, string $token, ?string $jsonBody): array
    {
        ...
        $lastFail = ['ok' => false, 'error' => 'curl failed', 'http_code' => 0];
        foreach ($authVariants as $authPair) {
            $headers = $authPair;
            if ($jsonBody !== null) {
            ...
            if (!is_string($body)) {
                $lastFail = ['ok' => false, 'error' => 'curl failed', 'http_code' => $code];

                continue;
            }
            if ($code < 200 || $code >= 300) {
                $lastFail = ['ok' => false, 'error' => 'HTTP ' . $code . ' ' . mb_substr($body, 0, 500), 'http_code' => $code];
            ...
            try {
                ...
                $lastFail = ['ok' => false, 'error' => 'json: ' . $e->getMessage(), 'http_code' => $code];
