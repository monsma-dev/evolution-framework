...
final class EvolutionFigmaService
{
    ...
    /**
     * Figma REST: prefer Bearer (current PAT docs); legacy tokens may still accept X-Figma-Token only.
     *
     * @return list<string>
     */
    private static function figmaRestHeaders(string $token): array
    {
        return [
    ...
    /**
     * Retry GET with legacy header when Bearer returns 4xx/5xx.
     *
     * @return array{ok: bool, data?: array<string, mixed>, error?: string, http_code?: int}|null
     */
    private static function figmaApiGetLegacyXToken(string $pathAndQuery, string $token): ?array
    {
        ...
        return is_array($data) ? ['ok' => true, 'data' => $data, 'http_code' => $code] : null;
    }
    ...
    private function figmaApiGet(string $pathAndQuery): array
    {
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_TIMEOUT => 40,
            CURLOPT_CONNECTTIMEOUT => 10,
            CURLOPT_HTTPHEADER => self::figmaRestHeaders($token),
        ]);
        $body = curl_exec($ch);
        $code = (int) curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);
        ...
        if ($code < 200 || $code >= 300) {
            $retry = self::figmaApiGetLegacyXToken($pathAndQuery, $token);
            if ($retry !== null) {
                return $retry;
            }
    ...
    {
        // Env first: server .env / process env must override stale tokens left in JSON.
        $fromEnv = trim((string) ($_ENV['FIGMA_ACCESS_TOKEN'] ?? getenv('FIGMA_ACCESS_TOKEN') ?: ''));
        if ($fromEnv !== '') {
            return $fromEnv;
        }
        ...
        $fig = $config->get('evolution.figma', []);
        $t = is_array($fig) ? trim((string) ($fig['api_token'] ?? '')) : '';
