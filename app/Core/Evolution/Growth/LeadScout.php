...
final class LeadScout
{
    ...
    {
        $dir = $this->basePath . '/data/evolution/growth';
        if (!is_dir($dir)) {
            mkdir($dir, 0750, true);
        }
    }
    ...
    /**
     * Generate crypto payment response for a lead interested in products.
     * Returns comment text with wallet address and payment instructions.
     */
    public function generatePaymentOffer(string $product = 'micro_consult'): ?string
    {
        $cfg = $this->loadCryptoConfig();
        if (!$cfg['enabled']) {
            return null;
        }
        ...
        $p = $products[$product];
        $template = $cfg['response_template'] ?? "Thanks for your interest! To proceed with **{product_name}**, please send **{price_eth} ETH** to:\n\n`{wallet_address}`\n\nNetwork: {network}\nExpected: ~$" . "{price_usd}\n\nOnce payment is detected (within 15 seconds), you'll receive access automatically.";

        return str_replace(
            ['{product_name}', '{price_eth}', '{price_usd}', '{wallet_address}', '{network}'],
            [$p['name'], (string)$p['price_eth'], (string)$p['price_usd'], $cfg['wallet_address'], $cfg['network']],
            $template
        );
    }

    /**
     * Post payment offer comment to GitHub issue/PR.
     * Returns true if posted successfully.
     */
    public function postPaymentComment(string $issueUrl, string $product = 'micro_consult'): bool
    {
        $text = $this->generatePaymentOffer($product);
        if (!$text) {
            return false;
        }

        // Extract owner/repo/issue from URL like https://github.com/owner/repo/issues/123
        if (!preg_match('/github\.com\/([^\/]+)\/([^\/]+)\/(issues|pull)\/(\d+)/', $issueUrl, $m)) {
            return false;
        }
        ...
        $repo  = $m[2];
        $num   = $m[4];
        $apiUrl = "https://api.github.com/repos/{$owner}/{$repo}/issues/{$num}/comments";

        $ch = curl_init($apiUrl);
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST           => true,
            CURLOPT_POSTFIELDS     => json_encode(['body' => $text]),
            CURLOPT_HTTPHEADER     => [
                'Accept: application/vnd.github+json',
        ...
        if ($status === 201) {
            $this->notifyTelegramPaymentOffer($product, $issueUrl);
            return true;
        }
        return false;
    }
    ...
    /**
     * Check for new payments to the wallet.
     * Returns array of new payment transactions with ETH amounts.
     */
    public function checkForPayments(): array
    {
        $cfg = $this->loadCryptoConfig();
        if (!$cfg['enabled']) {
            return [];
        ...
        // Use BaseRpcService to scan for new incoming transactions
        $rpc = new \App\Core\Evolution\Wallet\BaseRpcService($this->basePath);
        $address = $cfg['wallet_address'];

        // Get recent incoming transactions (last 50 blocks)
        $incoming = $rpc->getRecentIncoming($address, 50);

        // Filter to only new/unseen payments
        $seenFile = $this->basePath . '/data/evolution/growth/payment_txs.json';
        ...
        foreach ($incoming as $tx) {
            if (!isset($seen[$tx['hash']])) {
                $newPayments[] = $tx;
                $seen[$tx['hash']] = ['eth' => $tx['value_eth'], 'time' => time()];
            }
        }

        // Save updated seen list
        file_put_contents($seenFile, json_encode($seen, JSON_PRETTY_PRINT), LOCK_EX);

        // Notify if new payments found
        if (!empty($newPayments) && $cfg['telegram_notify']) {
            $this->notifyTelegramPayments($newPayments);
        }
    ...
    /**
     * Match payment to product based on amount (with tolerance).
     * Returns product key or null if no match.
     */
    public function matchPaymentToProduct(float $ethAmount): ?string
    {
        $cfg = $this->loadCryptoConfig();
        $products = $cfg['products'] ?? [];

        foreach ($products as $key => $product) {
            $expected = (float) $product['price_eth'];
            // 10% tolerance
            if ($ethAmount >= ($expected * 0.9) && $ethAmount <= ($expected * 1.1)) {
                return $key;
            }
        }
        return null;
    ...
    private function loadCryptoConfig(): array
    {
        if (!defined('BASE_PATH')) {
        ...
        $data = json_decode((string) file_get_contents($cfg), true);
        return $data['github']['lead_scout']['crypto_payments'] ?? ['enabled' => false];
    }

    private function notifyTelegramPaymentOffer(string $product, string $issueUrl): void
    {
        $telegram = new \App\Core\Evolution\Wallet\TelegramService();
        ...
        $text = "💰 <b>Payment Offer Sent</b>\n\n"
              . "Product: {$p['name']}\n"
              . "Price: {$p['price_eth']} ETH (~\${$p['price_usd']})\n"
              . "Issue: {$issueUrl}\n\n"
    ...
    private function notifyTelegramPayments(array $payments): void
    {
        $telegram = new \App\Core\Evolution\Wallet\TelegramService();
        if (!$telegram->isConfigured()) {
            return;
        ...
        $total = array_sum(array_column($payments, 'value_eth'));
        $count = count($payments);

        foreach ($payments as $tx) {
            $product = $this->matchPaymentToProduct($tx['value_eth']);
            $productText = $product ? " (Matched: {$product})" : "";

            $text = "🎉 <b>Payment Received!</b>\n\n"
                  . "Amount: {$tx['value_eth']} ETH{$productText}\n"
                  . "From: {$tx['from']}\n"
