 ...
 *       "repository": "jouw-org/evolution-curriculum",
 *       "branch": "main",
 ...
 *   }
 *
 * Env (override): GITHUB_TOKEN, GITHUB_ACADEMY_TOKEN, GITHUB_ACADEMY_OWNER, GITHUB_ACADEMY_REPO
 *
...
final class GitHubAcademyService
{
    ...
    /**
     * Vul owner/repo/token vanuit repository-string en env (GITHUB_TOKEN, GITHUB_ACADEMY_*).
     *
     * @param array<string, mixed> $config evolution.json academy.github
     * @return array<string, mixed>
     */
    public static function normalizeGithubConfig(array $config): array
    {
        $out = $config;

        $token = trim((string)($out['token'] ?? ''));
        if ($token === '') {
            $token = trim((string)(getenv('GITHUB_TOKEN') ?: getenv('GITHUB_ACADEMY_TOKEN') ?: ''));
        }
        $out['token'] = $token;

        $owner = trim((string)($out['owner'] ?? ''));
        $repo  = trim((string)($out['repo'] ?? ''));
        $repository = trim((string)($out['repository'] ?? ''));
        ...
        if ($owner === '') {
            $owner = trim((string)(getenv('GITHUB_ACADEMY_OWNER') ?: ''));
        }
        if ($repo === '') {
            $repo = trim((string)(getenv('GITHUB_ACADEMY_REPO') ?: ''));
        }
        $out['owner'] = $owner;
        $out['repo']  = $repo;

        $interval = $out['sync_interval_hours'] ?? $out['sync_interval'] ?? 24;
        if (is_string($interval) && strtolower($interval) === 'daily') {
            $out['sync_interval_hours'] = 24;
        } elseif (is_numeric($interval)) {
            $out['sync_interval_hours'] = max(1, (int)$interval);
        } else {
            $out['sync_interval_hours'] = 24;
        }
    ...
    {
        $config = self::normalizeGithubConfig($config);
     ...
     */
    public static function syncCurriculum(array $config, bool $force = false): array
    {
        $config = self::normalizeGithubConfig($config);
        if (!($config['enabled'] ?? false)) {
        ...
        // Sync-interval bewaken (CLI/UI: --force om te forceren)
