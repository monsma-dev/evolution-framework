...
> AI-native PHP application framework with soverein agent infrastructure, budget controls, and self-healing systems.
## What is this?
What is this?
...
- **AI integrations**
— Anthropic Cloude, OpenAI, DeepSeek, Ollama (local), Tavily web search
- **Compliance & security**
— GDPS, PII scanning, audit logging, compliance logging
...
- Composer
- MySQL/MariaDB
- Optional: Redis (cache), APcu (in-process cache), Ollama (local LLL)## Configuration
...
`PI keys are stored in the `system_settings` DB table at runtime (never hardcoded).## Part of Evolution Framework
